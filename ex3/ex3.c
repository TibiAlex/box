...
void reversePic(Picture *pic) {
         for(int i = 0; i < pic->height/2; i++) {
         	   for(int j = 0; j < pic->width; j++) {
                    int auxR, auxG, auxB;
                    auxR = GET_PIXEL(pic->pix_array, i ,j).R;
                    auxG = GET_PIXEL(pic->pix_array, i ,j).G;
                    auxB = GET_PIXEL(pic->pix_array, i ,j).B;
                    GET_PIXEL(pic->pix_array, i ,j).R = GET_PIXEL(pic->pix_array, pic->height - 1 - i ,j).R;
                    GET_PIXEL(pic->pix_array, i ,j).G = GET_PIXEL(pic->pix_array, pic->height - 1 - i ,j).G;
                    GET_PIXEL(pic->pix_array, i ,j).B = GET_PIXEL(pic->pix_array, pic->height - 1 - i ,j).B;
                    GET_PIXEL(pic->pix_array, pic->height - 1 - i ,j).R = auxR;
                    GET_PIXEL(pic->pix_array, pic->height - 1 - i ,j).G = auxG;
                    GET_PIXEL(pic->pix_array, pic->height - 1 - i ,j).B = auxB;
         	   }
         }
}
...
void colorToGray(Picture *pic) {
	  for(int i = 0; i < pic->height; i++) {
	  	  for(int j = 0; j < pic->width; j++) {
	  	  	     GET_PIXEL(pic->pix_array, i ,j).R = 0.3 * GET_PIXEL(pic->pix_array, i ,j).R;
	  	  	     GET_PIXEL(pic->pix_array, i ,j).G = 0.59 * GET_PIXEL(pic->pix_array, i ,j).G;
	  	  	     GET_PIXEL(pic->pix_array, i ,j).B = 0.11 * GET_PIXEL(pic->pix_array, i ,j).B;
	  	  }
	  }
}
